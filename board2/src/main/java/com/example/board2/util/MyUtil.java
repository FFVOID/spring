...
public class MyUtil {
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl) {
		StringBuffer sb = new StringBuffer(); //문자열 데이터를 자주 추가하거나 삭제할때 메모리 낭비의 방지를 위해 사용
		int numPerBlock = 5; // <이전 6 7 8 9 10 다음> 이전과 다음 사이의 숫자를 몇개 표시할지
		int currentPageSetup; // "<이전" 버튼에 들어갈 값
		int page; //그냥 페이지숫자(6 7 8 9 10)를 클릭했을때 들어갈 값
		...
		if(listUrl.indexOf("?") != -1) { //indexof 문자열이 포함되어있는지
			//'?'가 들어있다면(쿼리스트링이 있다면)
			listUrl += "&";
		} else { //쿼리스트링이 없으면
			listUrl += "?";
		...
		//currentPage가 (5~9) (10~14) (15~19) .. 일때 currentPageSetup 5 10 15 ..
		currentPageSetup = (currentPage / numPerBlock) * numPerBlock;
		
		//currentPager가 5 10 15 20..일때 currentPageSetup 0 5 10 15...
