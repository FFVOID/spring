...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {
	...
	private BooleanExpression regDtsAfter(String searchDateType) {
		...
		if(StringUtils.equals("all", searchDateType) || searchDateType == null) {
			...
			dateTime = dateTime.minusMonths(1);//현재 날짜로부터 1개월전
		} else if(StringUtils.equals("6m", searchDateType)) {
			dateTime = dateTime.minusMonths(6);//현재 날짜로부터 6개월전
		}
	...
	public Page<Item> getAdminItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
		...
		//select count(*) from item where reg_time = ? and item_sell_status = ? and item_nm(create_by) like %검색어% 
	...
	public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
		...
		/*  select item.id, item.itemNm, itemDetail, item_img.imgUrl, item.price 
		 *  from item, item_img
		...
		//dto 객체로 바로 받아올때는 1.컬럼과 dto객체의 필드가 일치해야한다 2.dto객체의 생성자에 @QueryProjection를 반드시 사용해야 한다
