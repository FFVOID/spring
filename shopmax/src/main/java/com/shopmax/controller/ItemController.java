...
public class ItemController {
	...
	/*
	  ...
	  //of(조회할 페이지의 번호: ★0부터시작(인덱스처럼), 한 페이지당 조회할 데이터 갯수) //url경로에 페이지가 있으면 해당 페이지
	  번호를 조회하도록 하고 페이지 번호가 없으면 0페이지를 조회 Pageable pageable =
	...
	public String itemDtl(@PathVariable("itemId") Long itemId, Model model) {
		...
		try {
			...
			model.addAttribute("errorMessage", "상품정보를 가져오는중 에러가 발생했습니다");
			model.addAttribute("itemFormDto", new ItemFormDto()); //에러발생시 비어있는 객체를 넘겨준다
